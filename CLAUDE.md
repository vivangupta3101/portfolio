...
- **StrahL** deck lives in `assets/images/strahl/` (`p00`–`p28`, 2382 wide; 1802 tall on
  p00/02/04/06/08/10/11/12, 1684 on the rest) — sliced from
  `uploads/Lighting Pdf for web_compressed.pdf` (29 pages) with `pdfslice-strahl.html`.
  It runs as an in-page reader (`#strahl` in `PROJECTS`, same as Möbius). Page bg `#f6e2c6`.
- **Blind Watchmaker** deck lives in `assets/images/watchmaker/` (`cover.png` 2880×1620,
  `p00`–`p11` 2880×1632, `p11` 1622) — sliced from `uploads/dc-10 for wesbite.pdf`
  (1920×14114.9 pt, one page) with `pdfslice-watchmaker.html`. Page bg `#eaeaea`.
  Renders are still placeholders in the source deck; visuals to be added later.
